...
const Gen: React.FC<{}> = () => {
  ...
  // Form参数
  const columns: ProColumns<Gen.PageListItem>[] = [
    {
    ...
    {
      title: '更新时间', width: 200, valueType: "dateTime", search: false, dataIndex: 'updateTime'
    },
    {
      title: '操作', fixed: 'right', width: 400, search: false, render: (dom, record) => {
        return (
          <>
            ...
            <Divider type="vertical"/>
            <Popconfirm
              onConfirm={() => refRemove([record.tableId])}
              title="确定删除吗"
            >
              <a href='#'>
                <Space>
            ...
            <Divider type="vertical"/>
            <Popconfirm
              onConfirm={() => refSynchDb(record.tableName)}
              title="确定强制同步结构吗"
            >
              <a href='#'>
                <Space>
            ...
            <Divider type="vertical"/>
            <Popconfirm
              onConfirm={() => batchGenCode([record.tableName])}
              title="确定生成吗"
            >
              <a href='#'>
                <Space>
  ...
  return (
    <PageContainer>
      <ProTable<Gen.PageListItem, Gen.PageParams>
        {...proTableConfigs}
        pagination={{
          // 是否允许每页大小更改
